...
DTrack::DTrack(KAL_MEAN &mean, KAL_COVA &covariance, int track_id, int n_init, int max_age, const FEATURE &feature)
{
    this->mean = mean;
...
void DTrack::predit(KalmanFilter *kf)
{
...
void DTrack::update(KalmanFilter *const kf, const DETECTION_ROW &detection)
{
...
void DTrack::mark_missed()
{
    if (this->state == TrackState::Tentative || this->time_since_update > this->_max_age)
    {
        this->state = TrackState::Deleted;
    }
}

bool DTrack::is_confirmed() const
{
    return this->state == TrackState::Confirmed;
}

bool DTrack::is_deleted() const
{
    return this->state == TrackState::Deleted;
}

bool DTrack::is_tentative() const
{
...
void DTrack::featuresAppendOne(const FEATURE &f)
{

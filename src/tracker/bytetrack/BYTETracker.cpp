...
BYTETracker::~BYTETracker()
{
 ...
 std::vector<STrack> BYTETracker::update(const  std::vector<Yolov8OutPut>& objects)
{
	...
	this->frame_id++;
	 ...
	 std::vector<STrack*> strack_pool; // ????????????????????????????????????????????????????????????lost??????
	...
	linear_assignment(dists, dist_size, dist_size_size, match_thresh, matches, u_track, u_detection); // ????lapjv???????????
	...
	// ???tracked_stracks_swap???????????????????????tracked_stracks?????????????????????

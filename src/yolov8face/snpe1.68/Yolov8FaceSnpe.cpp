...
void Yolov8FaceSnpe::postProcessing(std::vector<FACE_RESULT> & _results, float det_scale, bool padding) {
	...
	for(const auto& it : this->_out_data_ptr){
		...
		const int stride = (int)ceil((float)this->_model_input_height / feat_h);

		for (int i = 0; i < feat_h; i++) {
			for (int j = 0; j < feat_w; j++)
			{
				float conf = pdata[box_reg];
...
//                    std::unique_ptr<float[]> dfl_softmax(new float[reg_max]);
					...
					float xmin = std::max((cx - pred_ltrb[0]) / width_scale / det_scale, 0.f) ;
					float ymin = std::max((cy - pred_ltrb[1]) / higth_scale / det_scale, 0.f) ;
					float xmax = std::min((cx + pred_ltrb[2]) / width_scale / det_scale, float(img_input_size.width - 1));
					float ymax = std::min((cy + pred_ltrb[3]) / higth_scale / det_scale, float(img_input_size.height - 1));
					...
					{
						float kpt_x = (pdata[k * 3 + 1 + box_reg] * 2 + j)*stride / width_scale;
						float kpt_y = (pdata[k * 3 + 1 + 1 + box_reg] * 2 + i)*stride / higth_scale;
						float kpt_conf = sigmoid_x(pdata[k * 3 + 1 + 2 + box_reg]);

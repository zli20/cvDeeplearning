...
int Yolov8Onnx::getInference(cv::Mat& img, std::vector<DET_OUTPUT> & results)
{
	float det_scale;
...
void Yolov8Onnx::drawResult(cv::Mat& img, const std::vector<DET_OUTPUT>& results)
{
    if (results.empty()) {
        return ;
    }
...
void Yolov8Onnx::postProcessing(float* data, float det_scale, std::vector<DET_OUTPUT> & results) {
